...
const NavItem: React.FC<NavItemProps> = ({ icon, label, path, isActive, onClick, badge }) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center justify-center gap-1.5 w-14 h-14 transition-all duration-500 relative group ${isActive
      ? 'text-primary'
      : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-200'
      }`}
  >
    <div className={`relative flex items-center justify-center transition-all duration-500 ${isActive ? 'scale-110 -translate-y-2' : 'group-hover:-translate-y-1'
      }`}>
      ...
      <span className={`material-symbols-outlined text-[26px] leading-none transition-all duration-500 ${isActive ? 'filled drop-shadow-[0_4px_8px_rgba(13,148,136,0.4)]' : ''
        }`}>
        {icon}
      </span>
    ...
    <span className={`text-[9px] uppercase tracking-[0.15em] transition-all duration-500 absolute -bottom-1 ${isActive ? 'font-black opacity-100 translate-y-0' : 'font-bold opacity-0 translate-y-2'
      }`}>
      {label}
    </span>
