...
export default function CategoryGrid({ onSelect, selectedId, categories = [] }: CategoryGridProps) {
        ...
        <section className="mb-10 animate-fade-in">
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 select-none">
                {categories.map((cat) => {
                    ...
                    // Use route or id as the key for selection
                    ...
                    return (
                        ...
                        >
                            <div className={`size-14 rounded-2xl flex items-center justify-center transition-colors
                                 ${selectedId === catId ? `bg-${color}-100 text-${color}-600` : `bg-slate-50 dark:bg-gray-700 text-${color}-500 group-hover:bg-${color}-50`}
                            `}>
                            ...
                            <span className={`text-[10px] font-black uppercase tracking-tight text-center leading-tight max-w-[80px]
                                ${selectedId === catId ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-gray-400'}
                            `}>
                                {cat.title}
                            </span>
                        </button>
