...
const HeroSlider = ({ router, carouselData }: { router: any, carouselData: any }) => {
    ...
    <div className="relative w-full h-52 lg:h-64 overflow-hidden rounded-[2.5rem] shadow-xl group">
      {displaySlides.map((slide: any, index: number) => (
        <div
          ...
          onClick={() => slide.link_value ? router.push(slide.link_value) : null}
        >
  ...
  return (
    <div className="flex flex-col relative isolate min-h-screen pb-24 lg:pb-0 lg:max-w-7xl lg:mx-auto">
      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-2xl shadow-sm rounded-b-[2rem] transition-all duration-300 pb-2 lg:hidden border-b border-white/20 dark:border-gray-800">
        ...
        <div className="px-5 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => router.push('/notifications')} className="relative p-2.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-slate-600 dark:text-white">
            ...
            <button id="cart-icon-target" onClick={() => router.push('/cart')} className="relative p-2.5 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-lg hover:scale-105 transition-transform">
      ...
      <section className="px-4 lg:px-0 mt-4 animate-slide-up relative z-10">
        <HeroSlider router={router} carouselData={heroCarousel} />
      </section>
      ...
      <section className="mt-6 px-4 lg:px-0 animate-slide-up relative z-10" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="flex lg:grid lg:grid-cols-8 gap-4 overflow-x-auto no-scrollbar pb-4 pt-2 px-1 lg:pb-0">
          <CategoryPill onClick={() => router.push('/medicines')} icon="medication" label="Medicines" gradient="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-900/10" color="text-blue-600 dark:text-blue-400" />
          <CategoryPill onClick={() => router.push('/lab-tests')} icon="biotech" label="Lab Tests" gradient="bg-gradient-to-br from-teal-50 to-teal-100 dark:from-teal-900/30 dark:to-teal-900/10" color="text-teal-600 dark:text-teal-400" />
          <CategoryPill onClick={() => router.push('/doctors')} icon="stethoscope" label="Doctors" gradient="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/30 dark:to-purple-900/10" color="text-purple-600 dark:text-purple-400" />
          <CategoryPill onClick={() => router.push('/hospitals')} icon="local_hospital" label="Hospitals" gradient="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/30 dark:to-emerald-900/10" color="text-emerald-600 dark:text-emerald-400" />
          <CategoryPill onClick={() => router.push('/ambulance')} icon="ambulance" label="Ambulance" gradient="bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/30 dark:to-red-900/10" color="text-red-600 dark:text-red-400" />
          <CategoryPill onClick={() => router.push('/insurance')} icon="security" label="Insurance" gradient="bg-gradient-to-br from-cyan-50 to-cyan-100 dark:from-cyan-900/30 dark:to-cyan-900/10" color="text-cyan-600 dark:text-cyan-400" />
          <CategoryPill onClick={() => router.push('/surgeries')} icon="medical_services" label="Surgeries" gradient="bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/30 dark:to-indigo-900/10" color="text-indigo-600 dark:text-indigo-400" />
          <CategoryPill onClick={() => router.push('/wellness')} icon="self_improvement" label="Wellness" gradient="bg-gradient-to-br from-lime-50 to-lime-100 dark:from-lime-900/30 dark:to-lime-900/10" color="text-lime-600 dark:text-lime-400" />
        </div>
      </section>
      ...
      <section className="px-4 lg:px-0 mb-8 lg:mt-8 animate-slide-up relative z-10" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
          ...
          />
          <BentoCard
            onClick={() => router.push('/mother-baby')}
      ...
      <section className="mb-8 pl-4 lg:px-0 animate-slide-up relative z-10" style={{ animationDelay: '0.6s' }}>
        <SectionHeader title="Aesthetic Treatments" onSeeAll={() => router.push('/skin-hair')} />
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 lg:pr-0 lg:pb-0 snap-x snap-mandatory">
          {loadingSkin ? (
            Array(5).fill(0).map((_, i) => (
            ...
            (skinHairServices || []).map((srv) => (
              <div key={srv.id} onClick={() => router.push('/skin-hair')} className="snap-start shrink-0 relative min-w-[220px] h-[280px] rounded-[2rem] overflow-hidden group cursor-pointer shadow-md">
