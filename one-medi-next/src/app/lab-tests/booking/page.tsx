...
function LabBookingContent() {
    ...
    // Reconstruct state from URL params
    const testId = searchParams.get('testId');
    const testName = searchParams.get('testName');
    const price = searchParams.get('price');
    const mrpParam = searchParams.get('mrp'); // Avoid shielding conflict
    const centerName = searchParams.get('centerName');

    const test = testId ? { id: testId, name: testName } : null;
    const variant = price ? {
        price: parseFloat(price),
        mrp: mrpParam ? parseFloat(mrpParam) : parseFloat(price) * 1.5,
    ...
    if (!test || !variant) {
        return (
            <div className="min-h-screen bg-bg-light dark:bg-bg-dark flex flex-col items-center justify-center p-6 text-center text-slate-900 dark:text-white">
                ...
                <button onClick={() => router.push('/lab-tests')} className="mt-4 text-primary font-bold">Go Back</button>
            </div>
        );
    }
