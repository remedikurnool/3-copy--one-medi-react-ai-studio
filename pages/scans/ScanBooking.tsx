...
export default function ScanBooking() {
  ...
  // Hardcoded for now as per design but wired to cart
  const scanName = "MRI Brain Plain";
  const centerName = "Kurnool Diagnostic Center";
  const price = 3500;
  const mrp = 4000;

  const handleProceed = () => {
    addToCart({
      id: `scan-${scanName.replace(/\s+/g, '-').toLowerCase()}-${selectedDate}-${selectedTime}`,
      type: 'scan',
      name: scanName,
      price: price,
      mrp: mrp,
      ...
      metadata: {
        ...
        patientName: patient === 'self' ? 'Self' : 'Ramesh (Father)'
      }
    });
    navigate('/checkout');
  };
        ...
        <div className="flex-1 flex flex-col">
            ...
            <div className="flex flex-col items-stretch justify-start rounded-xl shadow-sm bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 overflow-hidden">
              <div className="flex flex-row p-3 gap-4 items-center">
                ...
                <div className="flex flex-col grow justify-center gap-1">
                  ...
                  <p className="text-lg font-bold leading-tight">{scanName}</p>
                  <p className="text-gray-500 dark:text-gray-400 text-sm font-medium leading-normal">{centerName}</p>
          ...
          <div className="flex flex-col px-4">
            ...
            <div className="flex gap-3 overflow-x-auto no-scrollbar pb-4">
              {/* Selected Date */}
              <div
                onClick={() => setSelectedDate('12 Oct')}
                className={`flex flex-col items-center justify-center min-w-[72px] h-20 rounded-xl shadow-md cursor-pointer shrink-0 border-2 transition-all ${selectedDate === '12 Oct' ? 'bg-primary text-white border-primary' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700'}`}
              >
                <p className={`text-xs font-medium uppercase tracking-wide ${selectedDate === '12 Oct' ? 'opacity-90' : 'text-gray-500'}`}>Today</p>
                <p className="text-2xl font-bold">12</p>
                <p className={`text-xs font-medium ${selectedDate === '12 Oct' ? '' : 'text-gray-500'}`}>Oct</p>
              </div>
              ...
              {[
                ...
                <div
                  key={item.date}
                  onClick={() => setSelectedDate(`${item.date} Oct`)}
                  className={`flex flex-col items-center justify-center min-w-[72px] h-20 rounded-xl cursor-pointer shrink-0 border transition-all ${selectedDate === `${item.date} Oct` ? 'bg-primary text-white border-primary shadow-md' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-primary/50'}`}
                >
                  <p className={`text-xs font-medium uppercase tracking-wide ${selectedDate === `${item.date} Oct` ? 'opacity-90' : 'text-gray-500'}`}>{item.day}</p>
                  <p className="text-2xl font-bold">{item.date}</p>
                  <p className={`text-xs font-medium ${selectedDate === `${item.date} Oct` ? '' : 'text-gray-500'}`}>Oct</p>
                </div>
              ))}
            </div>
