...
import { useDoctors, useDoctorSearch, useDoctorsBySpecialty } from '../../hooks/useDoctors';
...
export default function DoctorList() {
  ...
  const { data: searchResults, loading: loadingSearch } = useDoctorSearch(debouncedSearch);
  const { data: specialtyDoctors, loading: loadingSpecialty } = useDoctorsBySpecialty(
  ...
  const filteredDoctors = useMemo(() => {
    ...
    // If specialty selected, use specialty results
    if (selectedSpecialty !== 'All' && specialtyDoctors) {
      return specialtyDoctors;
    }
    // Default: all doctors
    return allDoctors || [];
  }, [allDoctors, searchResults, specialtyDoctors, debouncedSearch, selectedSpecialty]);

  const isLoading = loadingAll || (debouncedSearch && loadingSearch) || (selectedSpecialty !== 'All' && loadingSpecialty);
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="relative shrink-0">
                    <div
                      ...
                      style={{ backgroundImage: `url("${doc.image_url || 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?auto=format&fit=crop&w=200&q=80'}")` }}
                    ></div>
                  ...
                  <div className="flex flex-col flex-1 min-w-0">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-xs font-bold mt-1 line-clamp-1">
                      {doc.qualifications || 'MBBS, MD'} • {doc.experience_years || 0}+ years Exp
                    </p>
                    <div className="flex items-center gap-1 text-gray-400 text-[10px] font-bold mt-2 uppercase tracking-tight">
                      <span className="material-symbols-outlined text-[14px]">apartment</span>
                      <span className="truncate">{doc.hospital_affiliation || 'One Medi Hospital'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-slate-900 dark:text-white text-xl font-black tracking-tighter">₹{doc.consultation_fee || 500}</p>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); navigate(`/doctors/${doc.id}/book`); }}
                    className="flex-1 max-w-[140px] flex items-center justify-center rounded-xl h-11 bg-primary hover:bg-primary-dark text-white gap-2 text-xs font-black uppercase tracking-widest shadow-lg shadow-primary/25 transition-all"
                  >
                    Book Now
                  </button>
                </div>
