...
export default function DoctorBooking() {
  ...
  const { doctorId, variant } = location.state || { doctorId: undefined };
  ...
  // Fallback variant if not passed
  const currentVariant = variant || {
    price: doctor?.consultation_fee || 500,
    type: 'Clinic Visit',
    icon: 'apartment'
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    if (booking) {
      navigate('/bookings', { state: { newBookingId: booking.id } });
    } else {
  ...
  if (error || !doctor) {
    return (
      <div className="min-h-screen bg-bg-light dark:bg-bg-dark flex flex-col items-center justify-center text-center p-6 font-sans text-slate-900 dark:text-white">
        ...
        <button onClick={() => navigate('/doctors')} className="mt-4 text-primary font-bold">Browse Doctors</button>
      </div>
    );
  }
        ...
        <div className="p-4 bg-white dark:bg-gray-900">
          <div className="flex gap-4 items-center p-3 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm">
            <div
              ...
              style={{ backgroundImage: `url("${doctor.image_url || 'https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?auto=format&fit=crop&w=200&h=200'}")` }}
            ></div>
            <div>
        ...
        <div className="p-4 mt-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold">₹{currentVariant.price || doctor.consultation_fee}</span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            <div className="flex justify-between text-base font-bold">
              <span>Total Pay</span>
              <span>₹{currentVariant.price || doctor.consultation_fee}</span>
            </div>
          </div>
        </div>
