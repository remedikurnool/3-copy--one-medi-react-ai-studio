...
export default function DoctorBooking() {
  ...
  const [selectedDate, setSelectedDate] = useState('Mon, 14 Oct');
  ...
  const handleConfirm = () => {
    addToCart({
      id: `doc-${doctor.id}-${selectedDate}-${selectedTime}`,
      type: 'doctor',
      name: doctor.name,
      price: currentVariant.price,
      mrp: currentVariant.price, // Doctors usually don't have discount on fee in this model
      image: doctor.image,
      qty: 1,
      metadata: {
        slot: selectedTime,
      ...
      <div className="relative flex flex-col max-w-md mx-auto bg-white dark:bg-gray-900 shadow-xl min-h-screen">
        ...
        <div className="flex flex-col px-4 pt-2">
          ...
          <div className="flex gap-3 overflow-x-auto no-scrollbar pb-4">
            {['Mon, 14 Oct', 'Tue, 15 Oct', 'Wed, 16 Oct'].map((d, i) => (
              <button
                key={i}
              ...
              >
                ...
                <span className="text-xl font-bold">{d.split(',')[1].split(' ')[0]}</span>
              </button>
            ))}
          </div>
