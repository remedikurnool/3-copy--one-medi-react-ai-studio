...
import { SKIN_HAIR_SERVICES, HOME_CARE_SERVICES } from '../constants';
...
const HeroSlider = ({ navigate }: { navigate: any }) => {
  ...
  const slides = [
    ...
    {
      id: 2,
      title: "Expert Pregnancy Care",
      subtitle: "Start @ ₹15,999 | 9-Month Plan",
      tag: "Mother & Baby",
      bg: "from-rose-900 via-pink-800 to-purple-900",
      img: "https://images.unsplash.com/photo-1544126592-807daa215a75?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80",
      link: '/mother-baby',
    ...
    },
    {
      ...
      subtitle: "65+ Tests @ ₹999 | Home Sample",
      tag: "Diagnostics",
      bg: "from-blue-900 via-indigo-800 to-slate-900",
      img: "https://images.unsplash.com/photo-1579152276506-44439679bb4c?auto=format&fit=crop&w=800&q=80",
      link: '/lab-tests',
  ...
  return (
    <div className="relative w-full h-52 lg:h-64 overflow-hidden rounded-[2.5rem] shadow-xl group">
      {slides.map((slide, index) => (
        ...
        >
          <div className={`absolute inset-0 bg-gradient-to-r ${slide.bg} z-10`}></div>
          <img src={slide.img} className="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-60 transition-transform duration-[5000ms] ease-out scale-100 group-hover:scale-110" alt="" />
          ...
          <div className="absolute inset-0 z-20 p-8 flex flex-col justify-center">
            ...
            <p className={`text-sm lg:text-base font-bold ${slide.accent} mb-6`}>{slide.subtitle}</p>
...
export default function Home() {
  ...
  const { data: medicines, loading: loadingMedicines } = useMedicines(5);
  const { data: doctors, loading: loadingDoctors } = useDoctors(5);
  const { data: labTests, loading: loadingLabTests } = useLabTests(5);
  const { data: scans, loading: loadingScans } = useScans(5);
    ...
    <div className="flex flex-col relative isolate min-h-screen pb-24 lg:pb-0 lg:max-w-7xl lg:mx-auto">
        ...
        <div className="flex lg:grid lg:grid-cols-5 gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 lg:pr-0 lg:pb-0 snap-x snap-mandatory">
          {loadingMedicines ? (
            Array(5).fill(0).map((_, i) => (
              <div key={i} className="snap-start shrink-0 min-w-[180px] h-[240px] rounded-2xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
            ))
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 lg:pr-0 lg:pb-0 snap-x snap-mandatory">
          {loadingDoctors ? (
            Array(4).fill(0).map((_, i) => (
              <div key={i} className="snap-start shrink-0 min-w-[200px] h-[280px] rounded-2xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
            ))
              ...
              <div key={doc.id} className="snap-start shrink-0">
                <DoctorCard
                  doctor={{
                    ...
                    languages: doc.languages || ['English', 'Telugu'],
                    videoAvailable: doc.video_consultation || false,
                  }}
      ...
      <section className="mb-8 pl-4 lg:px-0 animate-slide-up relative z-10" style={{ animationDelay: '0.5s' }}>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 lg:pr-0 lg:pb-0 snap-x snap-mandatory">
          {loadingLabTests ? (
            Array(4).fill(0).map((_, i) => (
              <div key={i} className="snap-start shrink-0 min-w-[200px] h-[180px] rounded-2xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
            ))
              ...
              <div key={test.id} className="snap-start shrink-0">
                <LabTestCard
                  test={{
                    ...
                    mrp: 1500,
                    discount: '33%',
                    turnaround: `${test.report_time_hours || 24} hrs`,
                    fastingRequired: test.fasting_required || false,
                    image: test.image_url || 'https://images.unsplash.com/photo-1579154204601-01588f351e67?auto=format&fit=crop&w=200&q=80',
                  }}
      ...
      <section className="mb-24 pl-4 lg:px-0 animate-slide-up relative z-10" style={{ animationDelay: '0.7s' }}>
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 pr-4 lg:pr-0 lg:pb-0 snap-x snap-mandatory">
          {loadingScans ? (
            Array(4).fill(0).map((_, i) => (
              <div key={i} className="snap-start shrink-0 min-w-[200px] h-[200px] rounded-2xl bg-gray-100 dark:bg-gray-800 animate-pulse" />
            ))
              ...
              <div key={scan.id} className="snap-start shrink-0">
                <ScanCard
                  scan={{
                    id: scan.id,
                    name: scan.name,
                    modality: scan.modality || 'Scan',
                    bodyPart: scan.body_part || '',
                    price: 2500,
                    mrp: 3500,
                    discount: '28%',
                    duration: `${scan.duration_minutes || 30} mins`,
                    image: scan.image_url || 'https://images.unsplash.com/photo-1559757148-5c350d0d3c56?auto=format&fit=crop&w=200&q=80',
                  }}
