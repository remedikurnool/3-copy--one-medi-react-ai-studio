...
import { useLabTest, useLabTestPricing } from '../../hooks/useLabTests';
...
// Loading skeleton
const LabTestDetailSkeleton = () => (
   <div className="min-h-screen bg-bg-light dark:bg-bg-dark pb-28 animate-pulse">
      ...
      <div className="p-4">
         <div className="h-40 rounded-2xl bg-gray-200 dark:bg-gray-700 mb-4"></div>
...
export default function LabTestDetail() {
   ...
   // Fetch lab test from Supabase
   const { data: test, loading, error } = useLabTest(id);
   const { data: pricing } = useLabTestPricing(id);
   ...
   if (error || !test) {
      return (
         <div className="min-h-screen bg-bg-light dark:bg-bg-dark flex flex-col items-center justify-center p-8">
            <span className="material-symbols-outlined text-6xl text-gray-400 mb-4">science</span>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">Test not found</h2>
            <p className="text-gray-500 text-sm mb-6">The lab test you're looking for doesn't exist.</p>
            <button
   ...
   const currentPrice = selectedPricing ? selectedPricing.selling_price : test.base_price || 500;
   const currentMrp = selectedPricing ? selectedPricing.mrp : test.base_price || 600;
   ...
   const handleBookNow = () => {
      addToCart({
         id: test.id,
         type: 'lab',
         name: `${test.name} (${selectedPricing?.vendor_locations?.name || 'Lab'})`,
         price: totalAmount,
         mrp: currentMrp,
         qty: 1,
      ...
      <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-900 dark:text-white pb-28 relative flex flex-col font-sans">
         ...
         <main className="flex-1 w-full">
               ...
               <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary to-blue-400 p-6 shadow-lg text-white">
                  ...
                  <div className="relative z-10 flex flex-col gap-4">
                     ...
                     <div>
                        ...
                        <p className="mt-1 text-blue-50 text-sm font-medium">{test.description || 'Comprehensive health test'}</p>
                     </div>
                     <div className="mt-2 flex items-end gap-3">
                        ...
                        {currentMrp > currentPrice && (
                           <>
                              ...
                              <span className="mb-1 rounded-md bg-white px-2 py-0.5 text-sm font-bold text-primary">{discountPercent}% OFF</span>
                           </>
                        )}
                     </div>
                  </div>
            ...
            <div className="px-4 mb-4">
               <div className="grid grid-cols-2 gap-3">
                  <InfoCard icon="schedule" label="Report in" value={test.report_time || "24 Hours"} color="bg-blue-50 text-primary" darkColor="dark:bg-blue-900/20 dark:text-blue-400" />
                  <InfoCard icon="bloodtype" label="Sample" value={test.sample_type || "Blood"} color="bg-red-50 text-red-500" darkColor="dark:bg-red-900/20 dark:text-red-400" />
                  <InfoCard icon="no_food" label="Fasting" value={test.fasting_required ? "Required" : "Not Required"} color="bg-amber-50 text-amber-600" darkColor="dark:bg-amber-900/20 dark:text-amber-400" />
                  <InfoCard icon="science" label="Category" value={test.category || "General"} color="bg-emerald-50 text-emerald-600" darkColor="dark:bg-emerald-900/20 dark:text-emerald-400" />
               </div>
            </div>

            {/* Centre Selection from pricing */}
                  ...
                  <div className="flex flex-col gap-3">
                     {pricing.map((priceOption: any) => (
                        <div
                           ...
                           className={`bg-white dark:bg-gray-800 rounded-xl p-3 border transition-all cursor-pointer flex gap-3 relative ${selectedPricing?.id === priceOption.id
                           ...
                           <div className="flex-1 flex flex-col justify-center">
                              ...
                              <div className="flex justify-between items-end mt-1">
                                 <div className="text-xs text-gray-500 dark:text-gray-400">
                                    {test.report_time || '24 Hours'} Report
                                 </div>
                                 <span className="font-bold text-slate-900 dark:text-white">₹{priceOption.selling_price}</span>
                              </div>
                           </div>
                           {selectedPricing?.id === priceOption.id && (
            ...
            <div className="flex flex-col gap-3 px-4 pb-4">
               {test.parameters && (test.parameters as string[]).length > 0 && (
                  <DetailsAccordion title={`Parameters Included (${(test.parameters as string[]).length})`} defaultOpen badge="DETAILED">
                     <ul className="space-y-3">
                        {(test.parameters as string[]).map((param: string, idx: number) => (
               ...
               {test.preparation_instructions && (
                  <DetailsAccordion title="Preparation Instructions" defaultOpen={!test.parameters}>
                     <p className="whitespace-pre-line">{test.preparation_instructions}</p>
                  </DetailsAccordion>
               )}

               <DetailsAccordion title="About this Test">
                  <p>{test.description || 'This package is designed to screen for common health conditions. It includes detailed analysis of parameters to help diagnose or monitor your health status effectively.'}</p>
               </DetailsAccordion>
            </div>
         </main>
