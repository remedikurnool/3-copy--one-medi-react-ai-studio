...
const AMBULANCE_TYPES = [
  ...
  },
  {
    ...
    type: 'MORTUARY',
    icon: 'mode_cool', // Icon representing cold/freezer
    baseFare: 2500,
    perKm: 30,
  ...
  },
  {
    ...
    type: 'RENTAL',
    icon: 'kitchen', // Represents the box unit
    baseFare: 3500,
    perKm: 0, // Flat rental usually includes delivery within city limits
...
export default function AmbulanceBooking() {
  ...
  const totalPrice = useMemo(() => {
    ...
    return total;
  }, [selectedService, addons, showMedicalAddons]);
  ...
  return (
    <div className="flex flex-col h-screen bg-bg-light dark:bg-bg-dark font-sans relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-[35vh] z-0">
        ...
        <div className="absolute top-0 w-full p-4 flex justify-between items-center text-white">
           <button onClick={() => navigate(-1)} className="size-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center hover:bg-white/30 transition-colors">
               ...
               <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 -mx-2 px-2 snap-x">
                  {AMBULANCE_TYPES.map((type) => (
                     <button
                        ...
                        className={`relative min-w-[160px] p-4 rounded-2xl border-2 transition-all snap-start text-left flex flex-col gap-3 ${
                           selectedId === type.id 
