...
import { useSkinHairTreatments, ServiceMaster } from '../../hooks';
import { useCartStore } from '../../store/cartStore';
import { triggerCartAnimation } from '../../components/ui/FlyingCartAnimation';

// Static concern categories for filtering
const CONCERNS = [
   ...
   { label: 'Pigmentation', icon: 'auto_awesome', color: 'text-purple-500 bg-purple-50 dark:bg-purple-900/20' },
];
...
export default function SkinHairCare() {
   ...
   const { data: skinHairServices, loading, error } = useSkinHairTreatments();

   const filteredServices = (skinHairServices || []).filter((service: ServiceMaster) => {
      const matchesConcern = selectedConcern === 'All' ||
         service.name.toLowerCase().includes(selectedConcern.toLowerCase()) ||
         service.description?.toLowerCase().includes(selectedConcern.toLowerCase());
   ...
   return (
      <div className="min-h-screen bg-[#fffbf7] dark:bg-bg-dark font-sans text-slate-900 dark:text-white pb-24">
         ...
         <main className="p-4 flex flex-col gap-8 animate-fade-in">
            ...
            <section className="relative overflow-hidden rounded-[2.5rem] bg-slate-900 text-white shadow-xl cursor-pointer group" onClick={() => navigate('/doctors')}>
               ...
               <div className="relative z-20 p-6 pt-48 flex flex-col items-start">
                  ...
                  <h2 className="text-2xl font-black leading-tight mb-1">Consult Specialist</h2>
                  <p className="text-xs text-gray-300 font-medium mb-1 uppercase tracking-wide">Dermatology & Cosmetology</p>
            ...
            {!loading && activeTab === 'Treatments' && (
               <div className="flex flex-col gap-6 animate-slide-up">
                  ...
                  {filteredServices.map((srv: ServiceMaster) => (
                     <div key={srv.id} className="group relative flex flex-col gap-0 p-5 rounded-[2.5rem] bg-white dark:bg-gray-800 shadow-glass border border-amber-50 dark:border-gray-700 overflow-hidden hover:shadow-float transition-all duration-500">
                        <div className="flex gap-5">
                           <div className="size-28 rounded-3xl bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 shrink-0 overflow-hidden shadow-soft flex items-center justify-center">
                              <span className="material-symbols-outlined text-5xl text-amber-500/50">face_retouching_natural</span>
                           </div>
                           <div className="flex-1 flex flex-col justify-center min-w-0">
                              <div className="flex justify-between items-start mb-1">
                                 <span className="text-[9px] font-black uppercase text-amber-500 tracking-wider bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded">{srv.category}</span>
                                 <div className="flex items-center gap-1 text-[10px] font-bold text-amber-600">
                                    <span className="material-symbols-outlined text-[14px] filled">star</span> 4.8
                                 </div>
                              </div>
                              <h4 className="font-black text-lg leading-tight text-slate-900 dark:text-white truncate">{srv.name}</h4>
                              <p className="text-xs text-gray-500 font-medium mb-2">Kurnool Certified Clinics</p>

                              {srv.requirements && srv.requirements.length > 0 && (
                                 <div className="flex flex-wrap gap-1">
                                    {srv.requirements.slice(0, 2).map((b, i) => (
                        ...
                        <div className="mt-5 grid grid-cols-3 gap-2 py-4 border-y border-gray-100 dark:border-gray-700">
                           <div className="flex flex-col items-center text-center">
                              <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Category</span>
                              <span className="text-[10px] font-bold text-slate-700 dark:text-gray-300 leading-tight line-clamp-1">{srv.category}</span>
                           </div>
                           <div className="flex flex-col items-center text-center border-x border-gray-100 dark:border-gray-700">
                              ...
                              <span className="text-[10px] font-bold text-slate-700 dark:text-gray-300">{srv.duration_minutes || 30}m</span>
                           </div>
                           <div className="flex flex-col items-center text-center">
                              <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1">Type</span>
                              <span className="text-[10px] font-bold text-slate-700 dark:text-gray-300">{srv.is_home_service ? 'Home' : 'Clinic'}</span>
                           </div>
                        </div>

                        <div className="flex justify-between items-center mt-5">
                           <div>
                              <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Contact for</p>
                              <span className="font-black text-2xl text-slate-900 dark:text-white tracking-tighter">Quote</span>
                           </div>
                  ...
                  {filteredServices.length === 0 && (
                     <div className="text-center py-12 opacity-50">
                        ...
                        <p className="text-sm font-bold uppercase tracking-widest">No matching treatments</p>
                     </div>
                  )}
               </div>
            )}
            ...
            {!loading && activeTab === 'Products' && (
               <div className="flex flex-col gap-6 animate-slide-up">
                  <div className="flex justify-between items-end px-1">
                     ...
                     <span className="text-[10px] font-bold text-amber-600 uppercase">Coming Soon</span>
                  </div>

                  <div className="text-center py-12">
                     <div className="size-20 rounded-full bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center mx-auto mb-4 text-amber-400">
                        <span className="material-symbols-outlined text-4xl">inventory_2</span>
                     </div>
                     <p className="text-sm font-bold text-slate-500 uppercase tracking-widest">Product catalog coming soon</p>
                     <p className="text-xs text-gray-400 mt-2">Browse our treatments for now!</p>
                  </div>
               </div>
            )}
