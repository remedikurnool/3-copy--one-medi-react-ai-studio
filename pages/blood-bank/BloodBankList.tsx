...
export default function BloodBankList() {
   ...
   const filteredBanks = (bloodBanks || []).filter(b =>
      selectedGroup === 'All' || (b.available_groups || []).includes(selectedGroup)
   );
            ...
            {!loading && filteredBanks.map(bank => (
               <div key={bank.id} className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                  <div className="flex justify-between items-start mb-2">
                     ...
                     <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${bank.location?.is_open_24x7 ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                        {bank.location?.is_open_24x7 ? 'Open 24/7' : 'Timings Vary'}
                     </span>
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1 mb-4">
                     <span className="material-symbols-outlined text-[16px]">location_on</span>
                     {bank.location?.address || 'Kurnool'}
                  </p>

                  <div className="flex flex-wrap gap-2 mb-5">
                     {(bank.available_groups || []).map(bg => (
                  ...
                  <div className="flex gap-3">
                     ...
                     <a href={`tel:${bank.location?.phone || ''}`} className="flex-1 h-10 rounded-xl bg-red-500 text-white flex items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest shadow-md active:scale-95 transition-all">
