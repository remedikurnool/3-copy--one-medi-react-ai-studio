...
export default function HomeCareBooking() {
   ...
   const { service, plan, isHomeVisit, preferences } = location.state || {};
   const { addresses, profile } = useUserStore();
   ...
   if (!service) {
      return (
         <div className="min-h-screen flex items-center justify-center flex-col gap-4 bg-bg-light dark:bg-bg-dark text-slate-900 dark:text-white p-4 text-center">
            ...
            <button onClick={() => navigate('/')} className="text-primary font-bold bg-primary/10 px-6 py-3 rounded-xl mt-2">Go Home</button>
         </div>
      );
   }
   ...
   const handleConfirm = () => {
      addToCart({
         id: `home-care-${service.id}-${Date.now()}`,
         type: 'home_care',
         name: `${service.title} (${plan?.name || 'Standard'})`,
         price: totalAmount,
         mrp: totalAmount,
         qty: 1,
         metadata: {
            slot: selectedTime,
            date: selectedDate,
            centerName: isHomeVisit ? 'Home Service' : 'OneMedi Clinic',
            patientName: 'Self'
         }
      });
      navigate('/checkout');
            ...
            {isHomeVisit && (
               <section>
                  <div className="flex justify-between items-center mb-3">
                     ...
                     <button onClick={() => navigate('/profile/addresses')} className="text-xs font-bold text-primary">Add New</button>
                  </div>
                  <div className="flex flex-col gap-3">
                     {addresses.length > 0 ? addresses.map(addr => (
                        ...
                        <div onClick={() => navigate('/profile/addresses')} className="p-6 rounded-2xl border-2 border-dashed border-gray-300 dark:border-gray-700 flex flex-col items-center justify-center gap-2 text-gray-500 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
