...
import { usePhysiotherapyServices, ServiceMaster } from '../../hooks';
...
export default function PhysioList() {
  ...
  // Fetch physiotherapy services from Supabase
  const { data: physioServices, loading, error } = usePhysiotherapyServices();

  const filters = ['All', 'Ortho Care', 'Neuro Care', 'Sports Rehab', 'Post-Op'];

  const filteredServices = (physioServices || []).filter((service: ServiceMaster) => {
      ...
      {!loading && (
        <div className="flex flex-col gap-4 p-4 max-w-2xl mx-auto">
          {filteredServices.map((service: ServiceMaster) => (
            ...
            >
              <div className="w-full h-44 bg-gradient-to-br from-primary/30 to-secondary/30 relative flex items-center justify-center">
                <span className="material-symbols-outlined text-7xl text-white/50">physical_therapy</span>
                ...
                <div className="absolute bottom-3 left-3 text-white">
                  <div className="flex items-center gap-1 bg-black/30 backdrop-blur-sm px-2 py-0.5 rounded-lg text-xs font-bold w-fit mb-1">
                    <span className="material-symbols-outlined text-xs filled text-amber-400">star</span> 4.8
                    <span className="text-[10px] opacity-80 font-medium">(120+)</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col p-5 gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 text-xs leading-relaxed line-clamp-2 font-medium">
                    {service.description || 'Professional physiotherapy service available in Kurnool'}
                  </p>

                  {/* Requirements Tags */}
                  {service.requirements && service.requirements.length > 0 && (
                    <div className="flex gap-2 mt-2 overflow-hidden">
                      {service.requirements.slice(0, 3).map(c => (
                ...
                <div className="flex items-center justify-between mt-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-xl font-black text-primary">{service.duration_minutes || 60}</span>
                      <span className="text-[10px] font-bold text-gray-400">minutes / {service.unit || 'session'}</span>
                    </div>
                  </div>
                  <button
          ...
          {filteredServices.length === 0 && (
            <div className="text-center py-12 flex flex-col items-center gap-4">
              ...
              <p className="text-gray-500 font-medium">No physiotherapy services found.</p>
            </div>
          )}
        </div>
      )}
