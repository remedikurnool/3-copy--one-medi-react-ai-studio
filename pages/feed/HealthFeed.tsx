...
import { useHealthContent, HealthContent } from '../../hooks';
...
export default function HealthFeed() {
   ...
   const { data: content, loading, error } = useHealthContent();
   ...
   // Featured Item (First by view_count or first item)
   const featuredItem = useMemo(() => {
      if (!content || content.length === 0) return null;
      return content.find(c => c.view_count > 100) || content[0];
   }, [content]);
   ...
   return (
      <div className="min-h-screen bg-bg-light dark:bg-bg-dark text-slate-900 dark:text-white pb-24 font-sans">
         ...
         <main className="p-4 flex flex-col gap-6">
            ...
            {/* Featured Hero Card */}
            {!loading && featuredItem && activeTab === 'All' && !search && (
               <div className="relative h-[340px] rounded-[2.5rem] overflow-hidden shadow-xl group cursor-pointer animate-fade-in">
                  <img src={featuredItem.thumbnail_url || 'https://images.unsplash.com/photo-1532938911079-1b06ac7ceec7?auto=format&fit=crop&q=80&w=800'} alt={featuredItem.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <div className="absolute bottom-0 left-0 p-6 w-full">
                     ...
                     <div className="flex items-center gap-3 text-white/80 text-xs font-medium">
                        <div className="flex items-center gap-1.5">
                           ...
                           {featuredItem.author_name || 'OneMedi Team'}
                        </div>
                        <span>•</span>
                        <span>{featuredItem.read_time_minutes} min read</span>
                     </div>
                  </div>
               </div>
            )}
                  ...
                  {feedItems.map((item) => (
                     <div key={item.id} className="flex gap-4 group cursor-pointer">
                        ...
                        <div className="relative w-32 h-24 sm:w-40 sm:h-28 shrink-0 rounded-2xl overflow-hidden bg-gray-100 dark:bg-gray-800">
                           <img src={item.thumbnail_url || 'https://images.unsplash.com/photo-1532938911079-1b06ac7ceec7?auto=format&fit=crop&q=80&w=400'} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                        ...
                        <div className="flex-1 flex flex-col justify-between py-0.5">
                           ...
                           <div className="flex items-center gap-2 mt-2">
                              <span className="text-[10px] text-gray-500 font-medium">
                                 {item.author_name || 'OneMedi Team'}
                              </span>
                              <span className="size-0.5 bg-gray-300 rounded-full"></span>
                              <span className="text-[10px] text-gray-400 font-medium">{new Date(item.published_at).toLocaleDateString()}</span>
                           </div>
                        </div>
                     </div>
                  ))}
